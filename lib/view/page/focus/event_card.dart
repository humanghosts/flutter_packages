...
@immutable
class FocusCardArgs<T extends Event> extends ViewArgs {
  /// 状态
...
@immutable
class FocusCardDataSource<T extends Event> extends ViewDataSource {
  final T event;
  final EventConfig config;

  const FocusCardDataSource({required this.event, required this.config});
}

class FocusCardLogic<T extends Event> extends ViewLogic<FocusCardArgs<T>, FocusCardDataSource<T>> {
  /// 未删除的子事件
...
/// 事件卡片
class FocusCard<T extends Event> extends View<FocusCardLogic<T>> {
  FocusCard({
    required String key,
    required FocusCardArgs<T> args,
    required FocusCardDataSource<T> dataSource,
  }) : super(key: key, args: args, dataSource: dataSource, newLogic: FocusCardLogic<T>());
