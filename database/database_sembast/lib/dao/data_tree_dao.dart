...
class SembastDataTreeDao<T extends DataTreeModel> extends SembastDataDao<T> implements api.DataTreeDao<T> {
  ...
  Future<List> _getTreeIdList(api.Filter filter, api.Transaction tx, {bool? isLogicDelete}) async {
    ...
    for (RecordSnapshot record in recordList) {
      ...
      // 下级过滤条件
      filters.add(sembast.Filter.matches(DataTreeModel.fullPathKey, "^(${fullPath.replaceAll("|", "\\|")}\\|)[0-9a-zA-Z|]+\$"));
    }
    filters.insert(0, sembast.Filter.inList(DataModel.idKey, idList));
    return await store.findKeys(tx.getTx(), finder: Finder(filter: sembast.Filter.or(filters)));
  }
